...
OUTPUT_JSON = './results/pipeline2_table_qa_simple.json'
...
def run_pipeline_2():
    ...
    print(f"Starting Pipeline 2 (Direct Table-QA Baseline) using {MODEL_NAME}...\n")
    
    # Filter tasks to isolate chosen target domains
    ...
    for i, task in enumerate(evaluation_tasks):
        ...
        system_prompt = f"""Read the database and answer the question in a list of lists format:
Example format:
[["Mario", 30], ["Luigi", 25]]
Absolutely NO TEXT INTRODUCTION.
Database Tables:
{serialized_data}"""
    ...
    print(f"Pipeline 2 Baseline completed! Results saved in {OUTPUT_JSON}")
