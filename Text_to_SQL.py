import json
import sqlite3
import ollama
import os

MODEL_NAME = 'ibm/granite4.1:8b'
INPUT_JSON = './queries.json'
OUTPUT_JSON = './results/pipeline1_text_to_sql.json'
DB_BASE_DIR = './database'

TARGET_DATABASES = ['world_1', 'orchestra']

def load_schemas():
    schemas = {}
    for db in TARGET_DATABASES:
        conn = sqlite3.connect(f"{DB_BASE_DIR}/{db}/{db}.sqlite")
        cursor = conn.cursor()
        cursor.execute("SELECT sql FROM sqlite_master WHERE type='table';")
        schemas[db] = "\n\n".join([row[0] for row in cursor.fetchall() if row[0]])
        conn.close()
    return schemas

def run_pipeline():
    with open(INPUT_JSON, 'r', encoding='utf-8') as f:
        tasks = json.load(f)
        
    print(f"Starting Pipeline 1 (Text-to-SQL) using {MODEL_NAME}...\n")
    
    schemas = load_schemas()
    
    evaluation_tasks = [t for t in tasks if t['db_id'] in TARGET_DATABASES]
    
    for i, task in enumerate(evaluation_tasks):
        db_id = task['db_id']
        print(f"[{i+1}/{len(evaluation_tasks)}] DB: {db_id} | Question: {task['question']}")
        
        system_prompt = f"""You are an expert SQL developer. Write a valid SQLite query to answer the user's question.
Schema:
{schemas[db_id]}
Respond ONLY with the raw SQL query. Do not include markdown blocks, explanations, or text formatting."""

        response = ollama.chat(model=MODEL_NAME, messages=[
            {'role': 'system', 'content': system_prompt},
            {'role': 'user', 'content': task['question']}
        ])
        
        predicted_sql = response['message']['content'].strip()
        print(f"Generated Query:\n{predicted_sql}\n" + "-"*40)
        
        task['predicted_query'] = predicted_sql

    os.makedirs(os.path.dirname(OUTPUT_JSON), exist_ok=True)
    with open(OUTPUT_JSON, 'w', encoding='utf-8') as f:
        json.dump(evaluation_tasks, f, indent=4)
        
    print(f"Pipeline 1 completed! Results saved in {OUTPUT_JSON}")

if __name__ == "__main__":
    run_pipeline()